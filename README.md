...
| `grok-4.3-fast` | `fast` | `basic`，优先使用高等级账号池（与 grok-4.20-fast 等价） |
...
Console 配额采用**延迟恢复轮换策略**：每账号消耗至剩余 15 次时才启动 15 分钟恢复计时器。这使得评分机制能够自然地将流量切换到配额充足的账号，避免同一批账号被反复选中导致 429。后台每 30 秒自动巡检过期窗口并重置配额，无需等待下次请求触发。
