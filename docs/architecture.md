...
| 层 | 目录 | 职责 |
| :-- | :-- | :-- |
| 入口 | `cmd/gork` | 解析命令、启动 HTTP 服务、提供 `config validate/docs`、`account check`、`account sso-sweep` 与健康检查命令。 |
| 应用组装 | `app` | 加载配置、初始化生命周期 hook（含可选 SSO validation scheduler）、注册产品路由、挂载观测和安全中间件。 |
| 控制面 | `app/control` | 管理账号、模型、代理目录和后台可变状态；SSO 校验编排在 `control/account`（本地预检 + 会话探针 + ListModels 回退）。 |
| 数据面 | `app/dataplane` | 执行账号选择、反向协议、SSE/WS/gRPC-web 解析、上游传输与反馈；`protocol`/`transport` 提供 accounts.x.ai 会话探针与分类。 |
