...
const SCHEMA_DEF = [
  {
    id: 'system', label: '系统配置', labelKey: 'config.schema.tabs.system',
    groups: [
      {
        title: '基础访问控制', titleKey: 'config.schema.groups.baseAccess',
        section: 'app',
        fields: [
          { key: 'app_url', label: 'APP 访问地址', labelKey: 'config.schema.fields.appUrl.label', type: 'text', desc: '应用对外提供服务的根 URL，格式如 https://your-domain.com。本地代理模式必须配置。', descKey: 'config.schema.fields.appUrl.desc', wide: true },
          { key: 'app_key', label: 'APP 访问密码', labelKey: 'config.schema.fields.appKey.label', type: 'password', desc: '管理后台鉴权密码。修改后已有 Session 将立即失效，需重新登录。', descKey: 'config.schema.fields.appKey.desc', rand: true },
          { key: 'api_key', label: 'API 调用密钥', labelKey: 'config.schema.fields.apiKey.label', type: 'text', desc: 'OpenAI 兼容 API 的鉴权密钥。多个值请使用英文逗号分隔；留空则禁用鉴权。', descKey: 'config.schema.fields.apiKey.desc', rand: true },
          { key: 'webui_enabled', label: '启用 WebUI', labelKey: 'config.schema.fields.webuiEnabled.label', type: 'bool', desc: '开放 WebUI 功能入口。关闭后相关路由将返回 404。', descKey: 'config.schema.fields.webuiEnabled.desc' },
          { key: 'webui_key', label: 'WebUI 访问密码', labelKey: 'config.schema.fields.webuiKey.label', type: 'password', desc: 'WebUI 的访问鉴权密码。配置后访问时必须提供。', descKey: 'config.schema.fields.webuiKey.desc', rand: true, showIf: 'app.webui_enabled' },
        ]
      },
      {
        title: '日志配置', titleKey: 'config.schema.groups.logging',
        section: 'logging',
        fields: [
          {
            key: 'file_level',
            ...
            desc: '日志文件写入的最低等级，可独立于控制台配置。默认 INFO。',
  ...
  {
    id: 'features', label: '应用功能', labelKey: 'config.schema.tabs.features',
    groups: [
      {
        title: '基本功能设置', titleKey: 'config.schema.groups.features',
        section: 'features',
        fields: [
          { key: 'temporary', label: '临时会话', labelKey: 'config.schema.fields.temporary.label', type: 'bool', desc: '启用后请求不会写入 Grok 会话历史，等同于无痕对话。', descKey: 'config.schema.fields.temporary.desc' },
      ...
      {
        title: '媒体返回格式', titleKey: 'config.schema.groups.mediaFormat',
        section: 'features',
        fields: [
          {
            key: 'image_format',
            ...
            type: 'select',
            options: [
              { value: 'grok_url', label: 'URL（Grok 原生）', labelKey: 'config.schema.options.imageFormat.grokUrl' },
              { value: 'local_url', label: 'URL（本地代理）', labelKey: 'config.schema.options.imageFormat.localUrl', disabledWhen: 'no_app_url', disabledTip: '请先填写 APP 访问地址', disabledTipKey: 'config.disabledTip.appUrlRequired' },
              { value: 'grok_md', label: 'Markdown（Grok 原生）', labelKey: 'config.schema.options.imageFormat.grokMarkdown' },
          ...
          },
          {
            ...
            type: 'select',
            options: [
              { value: 'grok_url', label: 'URL（Grok 原生）', labelKey: 'config.schema.options.videoFormat.grokUrl' },
              { value: 'local_url', label: 'URL（本地代理）', labelKey: 'config.schema.options.videoFormat.localUrl', disabledWhen: 'no_app_url', disabledTip: '请先填写 APP 访问地址', disabledTipKey: 'config.disabledTip.appUrlRequired' },
              { value: 'grok_html', label: 'HTML（Grok 原生）', labelKey: 'config.schema.options.videoFormat.grokHtml' },
              { value: 'local_html', label: 'HTML（本地代理）', labelKey: 'config.schema.options.videoFormat.localHtml', disabledWhen: 'no_app_url', disabledTip: '请先填写 APP 访问地址', disabledTipKey: 'config.disabledTip.appUrlRequired' },
            ],
            desc: 'local_* 模式会先将视频下载至服务端，再通过本地 URL 代理分发；请确保已配置 APP 访问地址，并评估出站带宽消耗。',
  ...
  {
    id: 'schedule', label: '任务调度', labelKey: 'config.schema.tabs.schedule',
    groups: [
      {
        title: '配额刷新调度', titleKey: 'config.schema.groups.refreshSchedule',
        ...
        fields: [
          ...
          { key: 'usage_concurrency', label: '刷新并发数', labelKey: 'config.schema.fields.usageConcurrency.label', type: 'number', desc: '并发调用 usage 接口刷新额度时允许的最大并行数。', descKey: 'config.schema.fields.usageConcurrency.desc' },
        ]
      },
      {
        title: 'SSO 有效性检查', titleKey: 'config.schema.groups.ssoValidation',
        section: 'account.sso_validation',
        fields: [
          { key: 'enabled', label: '启用 SSO 自动剔除', labelKey: 'config.schema.fields.ssoValidationEnabled.label', type: 'bool', desc: '按固定间隔分批检查 SSO 有效性；连续失败达到阈值后会软删除账号，运行时不再选中该 SSO。', descKey: 'config.schema.fields.ssoValidationEnabled.desc', help: '检查规则：先刷新账号状态；刷新失败直接记为失败，不再拉取模型列表。刷新成功后再拉取模型列表，只有模型列表也拉取成功才算正常。\n清理规则：连续失败次数达到阈值后调用软删除，不物理删除数据库记录。\n建议：大规模 SSO 池先使用较小并发和批量，避免主动探测触发 upstream 限流。', helpKey: 'config.schema.fields.ssoValidationEnabled.help' },
          { key: 'interval_sec', label: '检查间隔（秒）', labelKey: 'config.schema.fields.ssoValidationInterval.label', type: 'number', min: 1, desc: '每轮分批检查的触发间隔。完整扫描耗时约为 ceil(SSO 总数 / 每批检查数量) × 检查间隔。', descKey: 'config.schema.fields.ssoValidationInterval.desc', showIf: 'account.sso_validation.enabled' },
          { key: 'batch_size', label: '每批检查数量', labelKey: 'config.schema.fields.ssoValidationBatchSize.label', type: 'number', min: 1, desc: '每次调度最多取出的 SSO 数量，用于把大号池拆成多段检查。', descKey: 'config.schema.fields.ssoValidationBatchSize.desc', showIf: 'account.sso_validation.enabled' },
          { key: 'concurrency', label: '检查并发数', labelKey: 'config.schema.fields.ssoValidationConcurrency.label', type: 'number', min: 1, desc: '单批内同时执行刷新和模型列表探测的最大并行数。', descKey: 'config.schema.fields.ssoValidationConcurrency.desc', showIf: 'account.sso_validation.enabled' },
          { key: 'max_failures', label: '连续失败阈值', labelKey: 'config.schema.fields.ssoValidationMaxFailures.label', type: 'number', min: 1, desc: '同一个 SSO 连续失败达到该次数后软删除；检查成功会清零失败计数。', descKey: 'config.schema.fields.ssoValidationMaxFailures.desc', showIf: 'account.sso_validation.enabled' },
        ]
      },
      {
        title: '凭证失效保护',
        section: 'account.invalid_credentials',
        fields: [
          { key: 'max_failures', label: '自动禁用阈值', type: 'number', min: 1, desc: '运行时发现 invalid credentials 连续达到该次数后才将账号标记为 expired；显式恢复会清零计数。' },
        ]
      },
      {
        title: '过期账号自动清理',
      ...
      },
      {
        ...
        fields: [
          { key: 'max_inflight', label: '单号并发上限', labelKey: 'config.schema.fields.maxInflight.label', type: 'number', desc: '单个账号同时在执行的请求上限，超过则选号路径跳过该号（两种模式共用）。', descKey: 'config.schema.fields.maxInflight.desc' },
        ]
      },
      {
        title: '批量操作并发', titleKey: 'config.schema.groups.batchConcurrency',
        section: 'batch',
        fields: [
          { key: 'nsfw_concurrency', label: '开启 NSFW 并发数', labelKey: 'config.schema.fields.nsfwConcurrency.label', type: 'number', min: 1, desc: '批量开启 NSFW 时的 token 级并发数；每个 token 需经历三步网络请求。', descKey: 'config.schema.fields.nsfwConcurrency.desc' },
          { key: 'refresh_concurrency', label: '刷新 Usage 并发数', labelKey: 'config.schema.fields.refreshConcurrency.label', type: 'number', min: 1, desc: '批量刷新账户额度时的 token 级并发数。', descKey: 'config.schema.fields.refreshConcurrency.desc' },
          { key: 'asset_upload_concurrency', label: '上传 Asset 并发数', labelKey: 'config.schema.fields.assetUploadConcurrency.label', type: 'number', desc: 'API 接收文件附件后触发的 Asset 上传任务，全局最大并发数，跨所有请求共享。', descKey: 'config.schema.fields.assetUploadConcurrency.desc' },
          { key: 'asset_list_concurrency', label: '查询 Asset 并发数', labelKey: 'config.schema.fields.assetListConcurrency.label', type: 'number', desc: 'list_assets 接口的全局最大并发数，跨所有请求共享。', descKey: 'config.schema.fields.assetListConcurrency.desc' },
          { key: 'asset_delete_concurrency', label: '删除 Asset 并发数', labelKey: 'config.schema.fields.assetDeleteConcurrency.label', type: 'number', min: 1, desc: 'delete_asset 接口的全局最大并发数，同时作为管理后台批量清理的 token 级默认并发值。', descKey: 'config.schema.fields.assetDeleteConcurrency.desc' },
        ]
      },
    ]
  },
  {
    id: 'proxy', label: '网络代理', labelKey: 'config.schema.tabs.proxy',
    groups: [
      {
        title: '出站代理设置', titleKey: 'config.schema.groups.egressProxy',
        ...
        fields: [
          {
            ...
            type: 'select',
            options: [
              { value: 'direct', label: '直连', labelKey: 'config.schema.options.proxyMode.direct' },
              { value: 'single_proxy', label: '代理（固定）', labelKey: 'config.schema.options.proxyMode.singleProxy' },
              { value: 'proxy_pool', label: '代理（轮转）', labelKey: 'config.schema.options.proxyMode.proxyPool' },
            ],
            desc: '控制出站流量代理策略。direct 为直连；single_proxy 使用固定代理；proxy_pool 在代理池中轮转。',
          ...
          },
          {
            ...
            type: 'text',
            wide: true,
            desc: 'API 流量的出站代理地址，支持 http、https、socks5 协议。',
          ...
          },
          {
            ...
            type: 'text',
            wide: true,
            desc: '图片、视频等媒体资源下载的专用代理地址；留空时回落至基础代理。',
          ...
          },
          {
            ...
            type: 'textarea',
            desc: 'API 流量代理池，每行一个代理 URL，请求时轮转使用。',
          ...
          },
          {
            ...
            type: 'textarea',
            desc: '媒体资源下载专用代理池，每行一个代理 URL；留空时回落至基础代理池。',
          ...
          },
          {
            ...
            type: 'bool',
            desc: '跳过代理服务器 TLS 证书校验，仅在使用自签名证书的代理时启用。',
        ...
        fields: [
          {
            ...
            type: 'select',
            options: [
              { value: 'none', label: '关闭', labelKey: 'config.schema.options.clearanceMode.none' },
              { value: 'manual', label: '手动', labelKey: 'config.schema.options.clearanceMode.manual' },
              { value: 'flaresolverr', label: 'FlareSolverr', labelKey: 'config.schema.options.clearanceMode.flaresolverr' },
              { value: 'byparr', label: 'Byparr', labelKey: 'config.schema.options.clearanceMode.byparr' },
            ],
            desc: '控制 Cloudflare Clearance 的获取方式。',
          ...
          },
          {
            ...
            type: 'text',
            desc: 'curl_cffi 浏览器指纹标识，需与 Cloudflare 期望的 JA3 指纹匹配，例如 chrome136。',
          ...
          },
          {
            ...
            type: 'text',
            wide: true,
            desc: '请求头中的 User-Agent，需与 Cloudflare 校验 Cookie 时所使用的浏览器保持一致。',
          ...
          },
          {
            ...
            type: 'text',
            wide: true,
            desc: 'Cloudflare 下发的完整 Cookie 字符串，应包含 cf_clearance 等字段。',
          ...
          },
          {
            ...
            type: 'text',
            wide: true,
            desc: 'FlareSolverr 实例的 HTTP 服务地址。',
          ...
          },
          {
            key: 'byparr_url',
            label: 'Byparr URL',
            labelKey: 'config.schema.fields.byparrUrl.label',
            type: 'text',
            wide: true,
            desc: 'Byparr 实例的 HTTP 服务地址。',
            descKey: 'config.schema.fields.byparrUrl.desc',
            showIf: { path: 'proxy.clearance.mode', values: ['byparr'] },
          },
          {
            key: 'timeout_sec',
            label: '挑战超时（秒）',
            ...
            type: 'number',
            desc: 'Clearance Provider 完成 Cloudflare 挑战允许的最长等待时长。',
            descKey: 'config.schema.fields.challengeTimeout.desc',
            showIf: { path: 'proxy.clearance.mode', values: ['flaresolverr', 'byparr'] },
          },
          {
            ...
            type: 'number',
            desc: 'Clearance Cookie 预热刷新任务的调度间隔。',
      ...
      {
        title: '重试策略', titleKey: 'config.schema.groups.retry',
        section: 'retry',
        fields: [
          { key: 'max_retries', label: '应用层重试上限', labelKey: 'config.schema.fields.maxRetries.label', type: 'number', desc: '请求失败后的应用层最大重试次数；0 表示不重试。适用于 chat.completions 与 responses 端点。', descKey: 'config.schema.fields.maxRetries.desc' },
          { key: 'on_codes', label: '应用层重试触发状态码', labelKey: 'config.schema.fields.retryCodes.label', type: 'text', desc: '收到指定 HTTP 状态码时触发应用层重试，多个值请使用英文逗号分隔，例如 429,503。', descKey: 'config.schema.fields.retryCodes.desc' },
          { key: 'reset_session_status_codes', label: '传输层会话重建状态码', labelKey: 'config.schema.fields.resetSessionCodes.label', type: 'text', desc: '收到指定 HTTP 状态码时自动重建传输层代理会话（Transport Session），多个值请使用英文逗号分隔。', descKey: 'config.schema.fields.resetSessionCodes.desc' },
        ]
      },
      {
        title: '请求超时设置', titleKey: 'config.schema.groups.timeouts',
        section: 'chat',
        fields: [
          { key: 'timeout', section: 'chat', label: '对话请求超时（秒）', labelKey: 'config.schema.fields.chatTimeout.label', type: 'number', desc: '对话与图像编辑请求的 HTTP 连接超时时间。', descKey: 'config.schema.fields.chatTimeout.desc' },
          { key: 'timeout', section: 'image', label: '图像请求超时（秒）', labelKey: 'config.schema.fields.imageTimeout.label', type: 'number', desc: '图像生成 WebSocket 请求的总超时时间。', descKey: 'config.schema.fields.imageTimeout.desc' },
          { key: 'stream_timeout', section: 'image', label: '图像流式空闲超时（秒）', type: 'number', desc: '图像生成 WebSocket 在流式接收阶段允许的最长连续空闲等待时长。' },
          { key: 'timeout', section: 'video', label: '视频请求超时（秒）', labelKey: 'config.schema.fields.videoTimeout.label', type: 'number', desc: '视频生成请求的统一超时时间。', descKey: 'config.schema.fields.videoTimeout.desc' },
          { key: 'timeout', section: 'voice', label: '语音请求超时（秒）', labelKey: 'config.schema.fields.voiceTimeout.label', type: 'number', desc: '语音请求的统一超时时间。', descKey: 'config.schema.fields.voiceTimeout.desc' },
          { key: 'timeout', section: 'nsfw', label: 'NSFW 处理超时（秒）', labelKey: 'config.schema.fields.nsfwTimeout.label', type: 'number', desc: 'set_birth_date 与 enable_nsfw 接口调用的超时时间。', descKey: 'config.schema.fields.nsfwTimeout.desc' },
          { key: 'upload_timeout', section: 'asset', label: '资产上传超时（秒）', labelKey: 'config.schema.fields.assetUploadTimeout.label', type: 'number', desc: 'Asset 上传接口的超时时间。', descKey: 'config.schema.fields.assetUploadTimeout.desc' },
          { key: 'download_timeout', section: 'asset', label: '资产下载超时（秒）', labelKey: 'config.schema.fields.assetDownloadTimeout.label', type: 'number', desc: 'Asset 下载接口的超时时间。', descKey: 'config.schema.fields.assetDownloadTimeout.desc' },
          { key: 'list_timeout', section: 'asset', label: '资产查询超时（秒）', labelKey: 'config.schema.fields.assetListTimeout.label', type: 'number', desc: 'Asset 查询接口的超时时间。', descKey: 'config.schema.fields.assetListTimeout.desc' },
          { key: 'delete_timeout', section: 'asset', label: '资产删除超时（秒）', labelKey: 'config.schema.fields.assetDeleteTimeout.label', type: 'number', desc: 'Asset 删除接口的超时时间。', descKey: 'config.schema.fields.assetDeleteTimeout.desc' },
        ]
      },
    ]
  },
...
function applyConfigI18n() {
  document.title = tr('config.pageTitle', null, 'Gork - 配置管理');
  SCHEMA = buildSchema();
...
// Currently supported: 'no_app_url' — true when app.app_url is empty.
...
function _getValue(section, key, field) {
  const value = _getByPath(_cfg, _fullPath(section, key));
  return value === undefined && field && Object.prototype.hasOwnProperty.call(field, 'default')
    ? field.default
    : value;
}
...
function _getCurrentValue(section, key, field) {
  ...
  return fp in _dirty ? _dirty[fp] : _getValue(section, key, field);
}
...
function renderInput(field, section, value) {
  ...
  const onChange = (v) => {
    ...
    const orig = _getValue(section, field.key, field);
  ...
  if (field.rand) {
    const btn = _el('button', { class: 'cfg-rand-btn', type: 'button', title: tr('config.randomize', null, '随机生成'), onclick: () => {
...
function renderRow(field, section, tabId) {
  const fp      = _fullPath(section, field.key);
  const value   = _getValue(section, field.key, field);
  const isDirty = fp in _dirty;
  ...
  if (section === 'app' && field.key === 'webui_enabled') {
    ...
    btn.disabled = !Boolean(_getCurrentValue(section, field.key, field));
...
function _refreshDirtyState({ autoNormalizeSelects = true, changedPaths = null } = {}) {
  ...
  document.getElementById('save-bar-msg').textContent =
    n === 0
      ? tr('config.unsavedDefault', null, '存在未保存的更改')
      : n === 1
        ? tr('config.unsavedSingle', { n }, '存在 1 项未保存的更改')
        : tr('config.unsavedMultiple', { n }, `存在 ${n} 项未保存的更改`);
  ...
  if (webuiLinkBtn) {
    ...
    webuiLinkBtn.title = enabled ? '打开 WebUI' : '启用 WebUI 后可打开';
...
async function doSave() {
  ...
  // "proxy.egress.mode" → {proxy:{egress:{mode:v}}}
  const patch = {};
  Object.entries(_dirty).forEach(([fp, v]) => _setByPath(patch, fp, v));

  try {
    showToast(tr('config.saving', null, '正在保存更改…'), 'info');
...
async function doResetDefaults() {
  const ok = confirm(tr('config.restoreDefaultsConfirm', null, '确定要将所有配置还原为默认值吗？此操作会清除当前已保存的自定义配置。'));
  if (!ok) return;

  const button = document.getElementById('btn-reset-defaults');
  if (button) button.disabled = true;
  try {
    showToast(tr('config.restoreDefaultsRunning', null, '正在还原默认配置…'), 'info');
    await _api('POST', '/config/reset');
    await load();
    showToast(tr('config.restoreDefaultsSuccess', null, '配置已还原为默认值'), 'success');
  } catch (e) {
    showToast(tr('config.restoreDefaultsFailed', { message: e.message }, `还原默认配置失败: ${e.message}`), 'error');
  } finally {
...
(async () => {
  try {
    ...
    showToast(tr('config.loadFailed', { message: e.message }, `配置页初始化失败: ${e.message}`), 'error');
  }
})();
