...
(() => {
  ...
  function detect() {
    ...
    var p;
    try {
      p = new URLSearchParams(location.search).get('lang');
    } catch (e) {}
    p = normalize(p);
    if (p) {
      try {
  ...
  function get(o, k) {
    ...
    for (var p = k.split('.'), i = 0; i < p.length; i++) {
      if (o == null) return;
      o = o[p[i]];
    }
    return o;
  ...
  function fetchLang(l) {
    return fetch('/static/i18n/' + l + '.json', { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : {}))
      .catch(() => ({}));
  }
  ...
  function t(k, p) {
    ...
    if (p)
      Object.keys(p).forEach((n) => {
        v = v.replace(new RegExp('\\{' + n + '\\}', 'g'), p[n]);
      });
    return v;
  }
  ...
  function init() {
    ...
    Promise.all([
      fetchLang('en'),
      fetchLang('zh'),
      lang === 'en' || lang === 'zh' ? Promise.resolve({}) : fetchLang(lang),
    ]).then((parts) => {
      data =
        lang === 'en' ? parts[0] : merge(merge(parts[0], parts[1]), parts[2]);
      ready = true;
      apply(document);
      queue.forEach((cb) => {
        cb();
  ...
  window.I18n = {
    ...
    getLang: () => lang,
    getLanguages: () =>
      LANG_CODES.map((code) => ({ code: code, label: LANGS[code].label })),
    onReady: (cb) => {
      if (ready) cb();
      else queue.push(cb);
    },
